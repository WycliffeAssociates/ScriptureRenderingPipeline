...
public class RenderingTrigger
{
	...
	public async Task RunAsync([ServiceBusTrigger("WACSEvent", "RenderingWebhook", IsSessionsEnabled = false, Connection = "ServiceBusConnectionString")] string rawMessage)
	{
		...
		// todo: git a richer repoRenderResult: 
	...
	private static async Task<RenderingResultMessage> RenderRepoAsync(WACSMessage message, ILogger log)
	{
		...
		var baseBlobUri = Utils.GetOutputClient().Uri.AbsolutePath;
		...
		List<BlobMeta> blobMetaList = null;
		try
		{
			...
			blobMetaList = await RenderContentAsync(log, repoType, rendererInput, outputDir);
			foreach (var item in blobMetaList)
			{
				item.Url = baseBlobUri + item.Url;
			}
			// todo: map over returned meta list and prepend the base uri to each relative path
		}
		catch (Exception e)
		{
		...
		return CreateSuccessfulResultMessage(message, timeStarted, rendererInput, repoType, blobMetaList);
	}

	private static RenderingResultMessage CreateSuccessfulResultMessage(WACSMessage message, DateTime timeStarted, RendererInput rendererInput, RepoType resourceType, List<BlobMeta> blobMetaList)
	{
		return new RenderingResultMessage(message)
		{
			Successful = true,
			...
			blobMetaList = blobMetaList,
			// todo: add renderedBlobs info about each blob with url that was rendered;  
			// username: message.User
			// repo: message.Repo
			// new List<renderedBlobMeta>() { 
			//                 new Student(){ Id = 1, Name="Bill"},
			/* 
			Url: (where in blob storage was it rendered)
			isRenderingOfWhole: bool
			blobByteSize: number
			sha256: string
			type: html (pdf etc;)
			 */
	...
	private static async Task RenderContentAsync(ILogger log, RepoType repoType, RendererInput rendererInput, IOutputInterface outputDir)
	{
		...
		List<BlobMeta> blobMetaList = await renderer.RenderAsync(rendererInput, outputDir);
		// todo: return some info from rendered
		return blobMetaList;
	}
