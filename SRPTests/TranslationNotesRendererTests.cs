...
".SanitizeNewlines();
        ...
        public async Task TestWithContent()
        {
            ...
            var renderer = new TranslationNotesRenderer();
            await renderer.RenderAsync(input, outputFileSystem);
            Assert.AreEqual(ExpectedResult.SanitizeNewlines(), outputFileSystem.Files["gen/01.html"].SanitizeNewlines());
            Assert.AreEqual(FrontMatterExpected.SanitizeNewlines(), outputFileSystem.Files["gen/front.html"].SanitizeNewlines());
